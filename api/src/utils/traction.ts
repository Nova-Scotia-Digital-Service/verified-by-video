...
import { logger as pinoLogger, PinoLogger } from './logger'
...
export const sendDrpcResponse = async (connId: string, threadId: string, response: any) => {
  const endpoint = `/drpc/${connId}/response`
  const message = { response, thread_id: threadId }

  logger.info(`Sending response to ${connId}, message = ${JSON.stringify(message)}`)
...
export const sendGenericMessage = async (connId: string, endpoint: string, message: any) => {
  const baseUrl = process.env.TRACTION_BASE_URL
  const url = new URL(endpoint, baseUrl).toString()
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      logger.error(`Error sending message: ${response.status} ${response.data}`)
    }
  } catch (error) {
...
export const offerCredential = async (offer: any) => {
  ...
  logger.info(`Sending offer to ${offer['connection_id']}, offer = ${JSON.stringify(offer)}`)
...
export const createSchema = async (schemaName: string, schemaVersion: string, attributes: string[]): Promise<any> => {
...
export const createPresentationRequest = async (presentationData: any): Promise<any> => {
  ...
  logger.info(`Creating presentation request = ${JSON.stringify(presentationData)}`)
...
export const createOobInvitation = async (label: string, goalCode?: string, attachments?: []): Promise<any> => {
